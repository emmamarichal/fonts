name: "BJ Cree"
designer: "SIL International"
license: "OFL"
category: "SANS_SERIF"
date_added: "2026-03-26"
fonts {
  name: "BJ Cree"
  style: "normal"
  weight: 400
  filename: "BJCree-Regular.ttf"
  post_script_name: "BJCree-Regular"
  full_name: "BJCree"
  copyright: "Copyright (c) 2000-2026 SIL Global (https://www.sil.org). Latin glyphs originally copyright (c) 2014-2015 (URW)++ Design and Development, with modifications copyright (c) 2024-2026 SIL Global."
}
fonts {
  name: "BJ Cree"
  style: "normal"
  weight: 500
  filename: "BJCree-Medium.ttf"
  post_script_name: "BJCree-Medium"
  full_name: "BJCree Medium"
  copyright: "Copyright (c) 2000-2026 SIL Global (https://www.sil.org). Latin glyphs originally copyright (c) 2014-2015 (URW)++ Design and Development, with modifications copyright (c) 2024-2026 SIL Global."
}
fonts {
  name: "BJ Cree"
  style: "normal"
  weight: 600
  filename: "BJCree-SemiBold.ttf"
  post_script_name: "BJCree-SemiBold"
  full_name: "BJCree SemiBold"
  copyright: "Copyright (c) 2000-2026 SIL Global (https://www.sil.org). Latin glyphs originally copyright (c) 2014-2015 (URW)++ Design and Development, with modifications copyright (c) 2024-2026 SIL Global."
}
fonts {
  name: "BJ Cree"
  style: "normal"
  weight: 700
  filename: "BJCree-Bold.ttf"
  post_script_name: "BJCree-Bold"
  full_name: "BJCree Bold"
  copyright: "Copyright (c) 2000-2026 SIL Global (https://www.sil.org). Latin glyphs originally copyright (c) 2014-2015 (URW)++ Design and Development, with modifications copyright (c) 2024-2026 SIL Global."
}
subsets: "canadian-aboriginal"
subsets: "latin"
subsets: "menu"
source {
  repository_url: "https://github.com/silnrsi/font-bjcree"
  commit: "7476fb691e64eff8956faf2bd68911cb3e1c65b7"
  archive_url: "https://github.com/silnrsi/font-bjcree/releases/download/v7.000/BJCree-7.000.zip"
  files {
    source_file: "OFL.txt"
    dest_file: "OFL.txt"
  }
  files {
    source_file: "BJCree-Regular.ttf"
    dest_file: "BJCree-Regular.ttf"
  }
  files {
    source_file: "BJCree-Medium.ttf"
    dest_file: "BJCree-Medium.ttf"
  }
  files {
    source_file: "BJCree-SemiBold.ttf"
    dest_file: "BJCree-SemiBold.ttf"
  }
  files {
    source_file: "BJCree-Bold.ttf"
    dest_file: "BJCree-Bold.ttf"
  }
  branch: "main"
}
stroke: "SERIF"
primary_script: "Cans"
